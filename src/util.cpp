...
char *load_file(const char *path) {
    ...
    rewind(file);
    char *data = reinterpret_cast<char*>(calloc(length + 1, sizeof(char)));
...
GLfloat *malloc_faces(int components, int faces) {
    return reinterpret_cast<GLfloat*>(malloc(sizeof(GLfloat) * 6 * components * faces));
}
...
GLuint make_shader(GLenum type, const char *source) {
    ...
    if (status == GL_FALSE) {
        ...
        GLchar *info = reinterpret_cast<GLchar*> (calloc(length, sizeof(GLchar)));
    ...
    unsigned char *data, unsigned int width, unsigned int height)
{
    ...
    unsigned char *new_data = reinterpret_cast<unsigned char*>(malloc(sizeof(unsigned char) * size));
...
int wrap(const char *input, int max_width, char *output, int max_length) {
    *output = '\0';
    char *text = reinterpret_cast<char*>(malloc(sizeof(char) * (strlen(input) + 1)));
    strcpy(text, input);
